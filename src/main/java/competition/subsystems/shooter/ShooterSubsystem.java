...
public class ShooterSubsystem extends BaseSetpointSubsystem<AngularVelocity, Double> {
    ...
    public ShooterSubsystem(XCANMotorController.XCANMotorControllerFactory xcanMotorControllerFactory,
        ...
        this.currentDuringShootingThreshold = this.propertyFactory.createPersistentProperty("Current During Shooting", 20.0);
    }
    ...
    public Current getHighestShooterCurrent() {
        return getHealthyShooterMotors().stream().map(XCANMotorController::getCurrent)
                .max(Comparator.comparingDouble(current -> current.in(Amps))).orElse(Amps.of(0)
        );
    }
